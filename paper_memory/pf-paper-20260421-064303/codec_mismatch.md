...
- Pre-Analysis Plan Status (paper specifies: UNCOMMITTED — must be committed by PREREGISTER before COMPUTE runs. COMPUTE gate will reject any run where this status is not COMMITTED in hypothesis_lock.)
- CODEAUDIT bidirectional audit requirement (paper specifies: required before WRITER writes paper)
- REVIEWER minimum score (paper specifies: 7/10 on methodology rubric)
- Maximum REVIEWER revision cycles (paper specifies: 3)
...
## verdict: WARN
severity: Minor
issue: description_ambiguous: 1 minor mismatches, 1 acknowledged deviations documented in DataPassport, 7 unverified params. WRITER will address in limitations section.
