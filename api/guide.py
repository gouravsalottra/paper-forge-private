...
def _infer_method_style(text: str, confirmatory: bool) -> str:
    ...
    if any(term in text for term in ["meta-analysis", "meta analysis", "pooled effect", "publication bias", "effect size synthesis"]):
        return "meta_analysis"
    if any(term in text for term in ["event study", "event window", "announcement", "fomc"]):
        return "event_study"
    if any(
        term in text
        for term in [
...
def _infer_evidence_route(text: str) -> str:
    # Extended keyword rules — used only as fallback when LLM inference is unavailable
    if any(term in text for term in ["simulation", "synthetic", "simulated", "generated by", "generates its own", "agent-based", "agent based", "multi-agent"]):
        return "simulation_generated"
    if any(term in text for term in ["upload", "proprietary", "my dataset", "private", "provided by researcher"]):
        return "upload"
    if any(term in text for term in ["text corpus", "earnings call", "transcript", "nlp", "sentiment"]):
        return "text_corpus"
    if "wrds" in text:
        return "public_fallback"
    if any(term in text for term in ["edgar", "filing"]):
...
def _llm_infer_blueprint_fields(
    payload: dict[str, Any],
) -> dict[str, Any]:
    """
    Use the LLM to classify research design dimensions.
    Returns empty dict on any failure — caller falls back
    to keyword rules.
    ...
    try:
        ...
        if result.get("research_stance") not in {"confirmatory", "exploratory"}:
            return {}
        if result.get("method_family") not in VALID_METHOD_FAMILIES:
            return {}
...
def _reviewer_gate(confirmatory: bool, method: str) -> dict[str, Any]:
    return {
        ...
        "method_focus": {
            ...
            "simulation": [
                "parameter lock before results",
                "generalisability claim validity",
                "simulation design justification",
                "output sensitivity to parameters",
                "warm-up period adequacy",
            ],
            "agent_based_model": [
                "agent strategy realism",
                "market clearing mechanism validity",
                "heterogeneity design",
                "results sensitivity to agent fraction",
            ...
            "network_analysis": [
                ...
                "temporal stability",
                "contagion mechanism validity",
            ],
            "text_analysis": [
                "sentiment approach validity",
                "look-ahead in text windows",
                "multiple testing across dictionary choices",
                "economic magnitude of text signals",
            ],
            "causal_identification": [
                "instrument validity and exclusion restriction",
                "parallel trends assumption",
                "SUTVA violations",
                "local vs average treatment interpretation",
            ],
            "meta_analysis": [
                "publication bias",
                "study heterogeneity sources",
...
def _leakage_policy(method: str) -> dict[str, Any]:
    rules = {
        ...
        "simulation": (
            "Simulation parameters must be locked in the Blueprint "
            "before results are observed. No parameter adjustment "
            "after seeing output distributions."
        ),
        "agent_based_model": (
            "Agent strategy parameters must be fixed before "
            "simulation runs. Post-hoc strategy adjustment is "
            "equivalent to p-hacking."
        ),
        "network_analysis": (
            "Network construction methodology and thresholds must "
            "be defined before centrality or contagion analysis. "
            "No graph pruning after seeing results."
        ),
        "text_analysis": (
            "Text window and sentiment approach must be locked "
            "before scoring. No vocabulary or window adjustment "
            "after seeing sentiment distributions."
        ),
        "causal_identification": (
            "Instrument and identification strategy must be "
            "declared before estimation. No instrument search "
            "after seeing first-stage results."
        ),
        "meta_analysis": (
            "Paper inclusion criteria must be locked before "
            "effect size extraction. No selective exclusion "
...
def _economic_significance(method: str) -> dict[str, str]:
    mapping = {
        ...
        "simulation": (
            "Report magnitude of difference between market "
            "conditions in standard deviation units. Report "
            "effect size with bootstrap confidence intervals."
        ),
        "agent_based_model": (
            "Report market impact in basis points and liquidity "
            "deterioration in spread widening. Report crash "
            "severity in maximum drawdown units."
        ),
        "network_analysis": (
            "Report systemic risk contribution as percentage "
            "of total system variance. Report contagion speed "
            "in nodes affected per period."
        ),
        "text_analysis": (
            "Report sentiment-return relationship in basis "
            "points per sentiment unit with economic bounds."
        ),
        "causal_identification": (
            "Report treatment effect in economically "
            "interpretable units. Never report p-values alone."
        ),
        "meta_analysis": (
            "Report pooled effect size with confidence interval "
            "and I-squared heterogeneity measure."
        ),
    }
...
def _data_fallback_policy(evidence: str) -> dict[str, Any]:
    if evidence == "simulation_generated":
        return {
            "wrds_status": "not applicable",
            "preferred_sequence": [
                "simulation engine output",
                "researcher upload of simulation results",
                "manual connector request for HPC output",
            ],
            "selected_route": "simulation_generated",
            "when_public_data_incomplete": (
                "Simulation studies generate their own data. "
                "The data quality gate checks simulation "
                "parameter completeness, not external data coverage."
            ),
        }
    return {
...
def _architecture_defaults(result: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # Apply LLM inferences, falling back to keyword rules
    confirmatory_from_llm = (
        llm_inferences.get("research_stance") == "confirmatory"
    )
    ...
    confirmatory = "confirmatory" in research_state or confirmatory_from_llm or any(
        word in text for word in [
            "test whether", "hypothesis", "causes", "outperform",
            "abnormal returns", "amplif", "predict", "does ",
...
def _fallback_blueprint(payload: dict[str, Any]) -> dict[str, Any]:
    topic = str(payload.get("topic") or "").strip()
    raw_text, text = _payload_text(payload)
    confirmatory = any(word in text for word in [
        "test whether", "hypothesis", "causes", "outperform",
        "abnormal returns", "amplif", "predict", "does ",
        "relative to", "compared to", "higher than", "lower than",
    ])
