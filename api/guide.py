...
def _infer_method_style(text: str, confirmatory: bool) -> str:
    # Registry rules are the fallback when LLM inference is unavailable.
    return infer_method_family(text, confirmatory)


def _infer_evidence_route(text: str) -> str:
    # Registry rules are the fallback when LLM inference is unavailable.
    return infer_evidence_route(text, _infer_method_style(text, True))
...
def _reviewer_gate(confirmatory: bool, method: str) -> dict[str, Any]:
    return {
        ...
        }.get(method, method_definition(method)["reviewer_focus"]),
...
def _leakage_policy(method: str) -> dict[str, Any]:
    ...
    if method in VALID_METHOD_FAMILIES:
        return {
            "method": method,
            "primary_rule": spec["leakage_rule"],
    ...
    return {
        "method": method,
        "primary_rule": rules.get(method, spec["leakage_rule"]),
        "failure_action": "block launch for hard leakage; otherwise require researcher-visible caveat and repair scope",
    }
...
def _economic_significance(method: str) -> dict[str, str]:
    spec = method_definition(method)
    if method in VALID_METHOD_FAMILIES:
        return {"method": method, "rule": spec["economic_rule"]}
    mapping = {
    ...
    return {"method": method, "rule": mapping.get(method, spec["economic_rule"])}
