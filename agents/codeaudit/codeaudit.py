...
from agents.conductor.exceptions import StructuredOutputError
...
class CodeAuditAgent:
    ...
    def run(self) -> dict:
        ...
        self._update_results_gate(flag)
    ...
    def _update_results_gate(self, flag: str) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                ...
                VALUES (
                    ?,
                    COALESCE((SELECT p_value_passes FROM results_gate WHERE run_id=?), FALSE),
                    COALESCE((SELECT seed_consistent FROM results_gate WHERE run_id=?), FALSE),
                    ?,
                    ?
                )
                ON CONFLICT(run_id) DO UPDATE SET
                    codeaudit_clean=excluded.codeaudit_clean,
