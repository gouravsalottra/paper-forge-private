...
class SigmaJob2:
    ...
    def run(self) -> dict:
        ...
        self._update_results_gate(
            p_value_passes=bool(primary_significance_pass),
            seed_consistent=bool(seed_consistency.get("consistent", False)),
        )
    ...
    def _update_results_gate(self, p_value_passes: bool, seed_consistent: bool) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, COALESCE((SELECT codeaudit_clean FROM results_gate WHERE run_id=?), FALSE), ?)
                ON CONFLICT(run_id) DO UPDATE SET
                    p_value_passes=excluded.p_value_passes,
                    seed_consistent=excluded.seed_consistent,
                    last_updated=excluded.last_updated
                """,
                (self.run_id, int(p_value_passes), int(seed_consistent), self.run_id, now),
            )
            conn.commit()
