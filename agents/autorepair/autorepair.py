"""AUTOREPAIR agent: autonomous code-paper mismatch resolver."""
...
class FixerAgent:
    ...
    def _parse_mismatches(self, mismatch_text: str) -> list[dict]:
        ...
        load_dotenv()
        client, model = get_client("AUTOREPAIR")

        prompt = (
            "Parse this CODEAUDIT mismatch report into a structured list.\n\n"
            ...
            "- PAP gate / hypothesis_lock gate / Pre-Analysis Plan Status check\n"
            "  (implemented in agents/aria/aria.py _check_forge_gate())\n"
            "- DataPassport SHA-256 signature\n"
            "  (implemented in agents/miner/miner.py write_data_passport())\n"
            "- CODEAUDIT bidirectional audit before WRITER\n"
            "  (enforced in agents/aria/aria.py CODEAUDIT loop)\n"
        ...
        for attempt in range(2):
            ...
            track_usage(
                resp,
                run_id=self.run_id,
                phase_name="AUTOREPAIR",
                agent_name="AUTOREPAIR",
                model=model,
    ...
    def _fix_missing_comment(self, mismatch: dict) -> dict:
        """Add explicit spec marker constant to improve CODEAUDIT traceability."""
    ...
    def _rerun_miner(self) -> None:
        """Re-run DATAPULL to refresh data after ticker/date fixes."""
        print("  [AUTOREPAIR] Re-running DATAPULL with updated spec...")
        import os
        ...
        print(f"  [AUTOREPAIR] DATAPULL complete: {result.get('result_flag')}")
    ...
    def _rerun_statsrun_job(self) -> None:
        ...
        result = agent.run()
        print(f"  [AUTOREPAIR] SIGMA JOB 2 complete: {result.get('result_flag')}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run AUTOREPAIR agent")
