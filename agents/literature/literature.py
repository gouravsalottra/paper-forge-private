"""LITERATURE agent: literature search and mapping."""
...
class LiteratureAgent:
    def __init__(self, run_id: str, paper_md_path: str, output_dir: str, db_path: str = "pipeline.db") -> None:
        self.run_id = run_id
        ...
        with get_db_connection(self.db_path) as conn:
            ...
            if {"result_id", "run_id", "phase_name", "agent_name", "status", "created_at"}.issubset(cols):
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (uuid.uuid4().hex, self.run_id, "LITERATURE", "LITERATURE", flag, now),
                )
